// 引擎，基础类，文字、图片对象
import { splitStringByte } from '../../utils/util';
...
const Utils = {
  ...
  wx_downloadFile: async (img, isHttps = true) => {
    const httpsImg = isHttps ? httpsImgUrl(img) : img;
    ...
    return new Promise((resolve) => {
      ...
      query
        ...
        .exec((res) => {
          ...
          const eg = new Engine2D(canvas, w, h);
          resolve(eg);
        });
    });
  }
  const ctx = wx.createCanvasContext('test_canvas', this);
...
export {
  Engine, CreateBox, CreateText, CreateCir, Utils, CreateTexts,
};
